  ...
  save(element: TypWrrl): void {
    this.dataService.aktualisiereWrrlTyp(element.typ, element.id, element.seefliess).subscribe(
      (response) => {
        ...
        this.snackBar.open('Phytoplankton-Typ erfolgreich gespeichert!', 'Schließen', {
          duration: 3000,
      ...
      (error) => {
        console.error('Fehler beim Speichern des Phytoplankton-Typs:', error);
        this.snackBar.open('Fehler beim Speichern des Phytoplankton-Typs!', 'Schließen', {
          duration: 3000,
          horizontalPosition: 'center',
          verticalPosition: 'top',
        });
